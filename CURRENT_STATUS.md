...
- ✅ **TASK-17**（2026-09-15 14:01）：Tauri 生产构建 + 打包——MSI + NSIS setup
  + 便携 zip 全部产出；release exe 启动实测通过；MSI 双击安装留用户验证；
  代码签名未配置（无证书，正式发布时按 RELEASE.md SOP 补齐）
...
- 完成：**TASK-17 生产构建**。`pnpm tauri build` 零 error（release 编译 9m15s，
  首次打包自动下载 WiX 3.14 + NSIS 3.11 并哈希校验）。产物：MSI
  `CrossBrain_0.1.0_x64_en-US.msi`（2.6 MB）、NSIS `CrossBrain_0.1.0_x64-setup.exe`
  （1.8 MB）、便携版 `dist-release/CrossBrain_0.1.0_portable.zip`（2.3 MB，内含
  `CrossBrain.exe` 5.5 MB）。
- 修改了文件：`docs/impl/TASK_BREAKDOWN.md`（TASK-17 实施记录 + 勾选清单）、
  `CURRENT_STATUS.md`、`.gitignore`（新增 `/dist-release/`）。
- 注意事项：
  1. **release exe 启动实测**：独立进程启动 → 进程存活、窗口标题 `CrossBrain` →
     正常关闭（MSI 安装的就是同一二进制）。
  2. **MSI 双击安装留用户执行**（系统级写入 Program Files/开始菜单，按惯例不自动化）；
     代码签名无证书未配置，MoTW 提示属预期。
  3. 便携 zip 用 Python `zipfile` 打包（PowerShell 从 Bash 调用被安全策略拦截）。

### 2026-09-15 13:05:42 — WorkBuddy（GLM-5.3-Flash）

- 完成：**TASK-21 工具接入（增补任务，用户产品想法落地）**。设置页新增
  「工具接入」区 + 「扫描本机工具」弹窗：只读探测 10 类工具（已适配 3 +
  Spike 预探测 7），复选框勾选保存——未适配工具只登记意愿（存
  `state.selected_tools`，`None`=默认三全启用，旧状态文件零迁移）；
  保存即过 ADR-15 断链闸门后按新范围立即同步。
- 关键改动：`filter_tools_with` 状态驱动同步范围；卸载刻意保持全量（去痕优先）；
  探针 UI/命令/测试移除（`skills_root()` 与 `remove_crossbrain_skill_dir()` 保留复用）。
- 验证：cargo test **142 passed / 0 failed**；pnpm build 通过；干跑 0 未通过；
  **真机 GUI 走查 11/11**（弹窗 10 工具、勾选交互、取消不保存）；真实数据零改动
  （状态文件 md5 前后一致、rules.md `f0b10675…`、硬链接组 links=4）。
- **GUI 走查新坑（重要）**：CDP `newPage()` 建的页面没有 `__TAURI_INTERNALS__`
  注入（Tauri 初始化脚本只注入自己的 webview）——普通页里应用照常渲染但
  invoke 全灭。走查必须先定位带注入的原生页面。另：曾误关真窗口页面导致
  「无法启动」假象，用多页面枚举 + 注入检测排查。
