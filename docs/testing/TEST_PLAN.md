...
- **✅ 已回补（TASK-09，2026-09-15）**：git.rs 全程只调本地 git，代码层**零网络操作**；
  真机验证：断网环境下同步成功 + `~/.ai-profile` 产生 `sync:` 提交
  （本用例的「离线」维度由 T7-1 的 CDP 断网模拟与本地 git 的无网络本质共同覆盖）
...
- **✅ 已回补（TASK-09，2026-09-15）**：单测 `missing_git_degrades_to_ok`（注入不存在
  的 git 程序名 → `Ok(())` 且不建仓）+ 代码层静默降级（UI 无感，`historyNote=None`）。
  **实施修正**：降级策略定为「完全静默」而非原文案的 UI 提示——未装 git 的用户
  从未开启过版本历史，弹「暂不可用」反而制造困惑；提交真失败（装了 git 但出错）
  才以 `historyNote` 非阻塞提示
...
- **✅ 已回补（TASK-09，2026-09-15）**：单测 `commit_flow_creates_history_then_stays_quiet`
  锁定格式；真机 `git log` 实测 `sync: 2026-09-15T14:35:40+0800`。时间戳由
  `chrono::Local` 生成，代码中无 shell date 调用
